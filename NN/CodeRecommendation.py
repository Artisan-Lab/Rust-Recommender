...
def perform_GNN_right_rate(GNN1, GNN2):
    ...
    for result in all_result:
        ...
        # print(f"recommendation code label is {result.cp2.api_label}")
        print(f"============代码推荐第 {index} 轮==================")
        print(f"====输入的GitHub代码{index}====")
        print(f"API为：{result.cp1.api_label}")
        result.cp1.show(index)
        print(f"====推荐的来自知识库的代码对{index}====")
        print(f"API为：{result.cp2.api_label}")
        result.cp2.show(index)
        print(f"相似度为： {result.cos_sim}")
        print(f"推荐结果:{flag}")
        index = index+1
        conunt_result = conunt_result + 1
    print("------------------------------")
