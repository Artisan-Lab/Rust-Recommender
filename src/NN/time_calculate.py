...
if __name__ == "__main__":
    ...
    for i in sorted_code :
        j = j+1
        list_time_generation.append(time_cost_generation_graph[i])
        list_time_model.append(time_cost[i])
        list_bar_lenth.append(sorted_code[i])
        list_bar_x.append(f'{j}')
        list_color.append('yellowgreen')
    
    fig.plot(list_bar_x,list_time_generation,color='orangered', marker='o', label='Generation')
    fig.plot(list_bar_x,list_time_model,color='dodgerblue', marker='o', label='Recommendation')
    fig.bar(list_bar_x,list_bar_lenth,color = list_color)

    legend_font = {'size': 8}

    fig.legend(loc='upper left',prop = legend_font ,bbox_to_anchor=(0.485, 0.95))

    plt.ylabel('Time Cost(ms) / Code Lines')

    plt.savefig('src/NN/figure/1.pdf', format = 'pdf', dpi = 600, bbox_inches = 'tight',pad_inches = 0.1)

    


    # print(code_hang_lines.__len__())
